...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  ...
  final List<Map<String, dynamic>> _patients = [
    {'id': '1', 'name': 'Nguyễn Văn An', 'phone': '0912345678', 'symptom': 'Đau đầu, chóng mặt', 'status': 'waiting', 'time': '10:30'},
    {'id': '2', 'name': 'Trần Thị Bình', 'phone': '0987654321', 'symptom': 'Ho kéo dài, sốt nhẹ', 'status': 'waiting', 'time': '10:45'},
    {'id': '3', 'name': 'Lê Hoàng Cường', 'phone': '0909090909', 'symptom': 'Đau bụng, buồn nôn', 'status': 'in_progress', 'time': '09:15'},
    {'id': '4', 'name': 'Phạm Minh Đức', 'phone': '0933333333', 'symptom': 'Mất ngủ, stress', 'status': 'completed', 'time': '08:00'},
    {'id': '5', 'name': 'Hoàng Thị Em', 'phone': '0944444444', 'symptom': 'Đau răng, sưng nướu', 'status': 'waiting', 'time': '11:00'},
  ];
  ...
  final List<Map<String, dynamic>> _messages = [
    {
      'isBot': true,
      'text': 'Chào Bác sĩ, tôi là trợ lý AI chuyên về y khoa. Tôi có thể hỗ trợ gì cho bác sĩ hôm nay?',
      'type': 'text',
    }
  ];
    ...
    return InkWell(
      ...
      child: Container(
        padding: const EdgeInsets.symmetric(
            horizontal: AppDimens.md, vertical: AppDimens.lg),
        decoration: BoxDecoration(
    ...
    return Align(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: isBot ? [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 5, offset: const Offset(0, 2))] : null,
        ),
        child: Text(
          msg['text'] ?? '',
  ...
  Widget _buildTypingIndicator() {
    return const Padding(
      ...
      child: Text('AI đang phản hồi...', style: TextStyle(fontSize: 12, color: Colors.grey, fontStyle: FontStyle.italic)),
    );
  }
          ...
          Expanded(
            child: Container(
              ...
              child: TextField(
                ...
                decoration: const InputDecoration(hintText: 'Nhập câu hỏi chuyên môn...', border: InputBorder.none),
              ),
            ),
          ),
          const SizedBox(width: 8),
  ...
  void _sendDoctorMessage() {
    ...
    Future.delayed(const Duration(seconds: 1), () {
      ...
      setState(() {
        ...
        _messages.add({
          'isBot': true,
          'text': 'Theo cơ sở dữ liệu y khoa, về vấn đề "$text", bác sĩ có thể tham khảo phác đồ điều trị của Bộ Y Tế hoặc các nghiên cứu lâm sàng mới nhất. Tôi có thể tìm thêm thông tin chi tiết nếu bác sĩ cần.',
        });
      });
    });
  }
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                Text(patient['symptom'], style: const TextStyle(color: Colors.grey, fontSize: 13)),
                const SizedBox(height: 8),
                Row(
                  children: [
                    const Icon(Icons.phone_outlined, size: 14, color: Colors.grey),
                    const SizedBox(width: 4),
                    Text(patient['phone'], style: const TextStyle(fontSize: 12, color: Colors.grey)),
                    const SizedBox(width: 16),
                    const Icon(Icons.schedule_outlined, size: 14, color: Colors.grey),
                    const SizedBox(width: 4),
                    Text(patient['time'], style: const TextStyle(fontSize: 12, color: Colors.grey)),
                  ],
                ),
              ],
            ),
